...
var bank_account_currency_country_name = null;
...
$(document).ready(function () {
    ...
    // Load saved draft data for all forms
    $('.draftForm').each(function() {
      loadDraft($(this));
      LoadData();
    });

    // Save draft when a field loses focus or changes in any form
    $('.draftForm').on('blur change', 'input, select', function() {
        saveDraft($(this).closest('form'));
    });

    // Clear draft data when any "Clear Draft" button is clicked
    ...
    function saveDraft(form) {
        ...
        form.find('input[type="checkbox"]').each(function() {
            draftData[$(this).attr('name')] = $(this).is(':checked');
        });

        // Add selected values for select elements
    ...
    function loadDraft(form) {
        ...
        if (savedDraft) {
            ...
            $.each(draftData, function(key, value) {
                ...
                if (field.is(':radio')) {
                    ...
                    // Files cannot be set programmatically
                    // To handle files, consider re-uploading or notifying user
                } else {
                    field.val(value);
                }
            });
    ...
    //     form[0].reset();
    //     // Additionally clear checkboxes and file inputs manually if needed
...
function WizardFormEvent(){
    ...
    $("#onGotoBusinessDetails").click(function(){
      if ($('#stakeholder_contact_photo').val()) {
    $(".iti__flag-container").css({
      'margin-top': '-6px'
    });
    ...
    if (stakeholder_form_valid){
      ...
      $('#append_stakeholder_details').valid()
      Lobibox.notify('warning', {
        position: 'top right',
...
$('#stakeholder_contact_photo').on('change', function (e) {
  var files = e.target.files;
  for (var i = 0; i < files.length; i++) {
    var reader = new FileReader();
    reader.readAsDataURL(files[i]);
    (function (currentReader, currentFile) {
      currentReader.onload = function () {
        ...
        var replaceValue = inputData.split(',')[0];
        var fileData = inputData.replace(replaceValue, "");
        stakeHolderImg.push(fileData);
        if (stakeHolderImg.length === files.length) {
          console.log(stakeHolderImg);
        }
      };
    })(reader, files[i]);
  }
...
function DataValues() {
    $("#business_zip_code,#business_city,#business_state_province,#supplier_company_name, #supplier_registerd_address, #zip_code_or_postal_code, #supplier_factory_address, #supplier_year_established, #supplier_law, #supplier_date, #supplier_place, #supplier_organization, #supplier_company_email, #supplier_website_link, #supplier_telephone_no, #supplier_mobile_no,#stakeholder_partner_name, #stakeholder_chief_executive_name, #stakeholder_chief_executive_email, #stakeholder_chief_executive_phone, #stakeholder_manager_name, #stakeholder_manager_email, #stakeholder_manager_phone, #stakeholder_contact_person_name, #stakeholder_contact_photo, #stakeholder_contact_person_email, #stakeholder_contact_person_phone,#name_of_the_bank, #bank_info_bank_address, #bank_info_postal_code, #bank_acc_holder_name, #company_registration_no, #tax_id,#factory_locations, #production_capacity, #bank_info_postal_code, #no_of_plants, #no_of_warehouses").on("input", function () {
        ...
        $("#contact_photo_value").text(stakeholderContactPhoto);
...
function SupplierPartnerSubmitBtn(){
    ...
    $.ajax({
        ...
        data: {
          ...
          'stack_holder_photo': stakeHolderImg,
...
//     stakeholder_contact_photo:{required:"Select Profile Photo"},
...
$('#append_stakeholder_details').validate({
  rules: {
    ...
    stakeholder_contact_photo: { required: true },
    stakeholder_contact_person_email: { required: true },
    stakeholder_manager_phone: {
      required: true,
      maxlength: 15,
      minlength: 7
    },
    stakeholder_contact_person_phone: {
      required: true,
      maxlength: 15,
      minlength: 7
    },
  ...
  messages: {
    ...
    stakeholder_contact_person_email: { required: "Enter Contact Person Email" },
    stakeholder_manager_phone: {
      required: "Enter Manager Phone",
      maxlength: "Enter Valid Mobile Number",
      minlength: "Enter Valid Mobile Number",
    },
    stakeholder_contact_person_phone: {
      required: "Enter Contact Person Phone",
      maxlength: "Enter Valid Mobile Number",
      minlength: "Enter Valid Mobile Number",
    },
  },
...
//         draftData.financial_manager_name||draftData.financial_manager_email||draftData.financial_manager_phone||draftData.stakeholder_contact_person_name||draftData.stakeholder_contact_person_email||
